...
// Expediente de documentos de un evento (Panatickets): checklist, contrato,
// slots de subida y formulario de comprobantes. Lo usan el admin
// (/admin/eventos/[id]/documentos, puede firmar) y los usuarios de la empresa
// (/usuario/documentos/[id], suben todo pero la firma queda al gerente).
...
export function ExpedienteDocumentos({ eventoId, puedeFirmar, basePath, volverHref }: {
  eventoId: string
  puedeFirmar: boolean
  basePath: string      // prefijo de las vistas imprimibles: `${basePath}/formulario` y `${basePath}/planilla`
  volverHref: string
}) {
  ...
  const cargar = useCallback(async () => {
    const [ev, ds, ct, fm] = await Promise.all([
      ...
      fetch(`/api/eventos/${eventoId}/documentos`).then(r => r.json()),
      fetch(`/api/eventos/${eventoId}/contrato`).then(r => r.json()),
      fetch(`/api/eventos/${eventoId}/formulario`).then(r => r.json()),
    ])
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <button onClick={() => router.push(volverHref)} className="text-sm text-gray-500 hover:text-gray-800">
          ← Volver
        </button>
      ...
      <SeccionContrato eventoId={eventoId} contrato={contrato} onChange={setContrato} puedeFirmar={puedeFirmar} />
...
function SeccionContrato({ eventoId, contrato, onChange, puedeFirmar }: {
  eventoId: string
  ...
  return (
    <div className={`card p-5 border-2 ${firmado ? 'border-green-200' : contrato ? 'border-amber-200' : 'border-gray-200'}`}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {!firmado && !puedeFirmar && (
              <span className="text-xs text-amber-600 font-medium">
                ⏳ El gerente general firmará este contrato — ya fue notificado.
              </span>
            )}
      ...
      {verFirma && puedeFirmar && <div className="mt-4"><FirmaPad /></div>}
      {error && <p className="text-red-500 text-sm mt-3">{error}</p>}

      <input ref={fileRef} type="file" accept=".pdf" className="hidden"
        onChange={e => { const f = e.target.files?.[0]; if (f) subir(f); e.target.value = '' }} />

      {mostrarFirma && contrato && puedeFirmar && (
        <FirmarContrato
          eventoId={eventoId}
...
function SeccionFormulario({ eventoId, form, setForm, basePath }: {
  eventoId: string
  form: Formulario
